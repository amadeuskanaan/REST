...
def ica_aroma_denoise(fslDir, inFile, mask, dim, TR, mc, denType, affmat, warp):
    ...
    def register2MNI(fslDir, inFile, outFile, affmat, warp):
        """ This function registers an image (or time-series of images) to MNI152 T1 2mm.
         If no affmat is defined, it only warps (i.e. it assumes that the data has been registerd to the structural scan associated with the warp-file already).
         If no warp is defined either, it only resamples the data to 2mm isotropic if needed (i.e. it assumes that the data has been registered to a MNI152 template).
         In case only an affmat file is defined, it assumes that the data has to be linearly registered to MNI152 (i.e. the user has a reason not to use non-linear
    ...
    def classification(outDir, maxRPcorr, edgeFract, HFC, csfFract):
        """ This function classifies a set of components into motion and non-motion components based on four features;
    ...
    print '  2a- extracting the CSF & Edge fraction features'
    edgeFract, csfFract = feature_spatial(fslDir, outDir, melIC_MNI)

    print '  2b- extracting the Maximum RP correlation feature'
    melmix = os.path.join(outDir,'melodic.ica','melodic_mix')
    maxRPcorr = feature_time_series(melmix, mc)

    print '  2c- extracting the High-frequency content feature'
